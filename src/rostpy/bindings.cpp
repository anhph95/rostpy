...
void create_rost(py::module_ m, const char* name) {
        ...
        .def(py::init(
            ...
            return new R(V, K, alpha, beta, PoseNeighborsT(1), PoseHashT(), gamma);
        ...
        .def("perplexity",  static_cast<double (R::*)(const typename R::pose_t&, bool)>(&R::perplexity))
        ...
        .def("add_observation", static_cast<void (R::*)(const typename R::pose_t&, const std::vector<int>&)>(&R::add_observation))
        .def("add_observation", static_cast<void (R::*)(const typename R::pose_t&, const std::vector<int>::iterator&, const std::vector<int>::iterator&, bool)>(&R::add_observation))
        .def("add_observation", static_cast<void (R::*)(const typename R::pose_t&, const std::vector<int>::iterator&, const std::vector<int>::iterator&, bool, const std::vector<int>::iterator&, const std::vector<int>::iterator&)>(&R::add_observation))
        .def("forget", &R::forget)
...
PYBIND11_MODULE(_rostpy, m)
        {
                ...
                typedef ROST<pose_txy, pose_neighbors_txy, pose_hash_txy, pose_equal_txy> ROST_txy;
                ...
                create_rost<pose_t, pose_neighbors_t, pose_hash_t, pose_equal_t, ROST_t>(m, "ROST_t");
                m.def("parallel_refine", parallel_refine<ROST_t>);
                create_rost<pose_txy, pose_neighbors_txy, pose_hash_txy, pose_equal_txy, ROST_txy>(m, "ROST_txy");
                m.def("parallel_refine", parallel_refine<ROST_txy>);
                create_rost<pose_xy, pose_neighbors_xy, pose_hash_xy, pose_equal_xy, ROST_xy>(m, "ROST_xy");
